...
def batch_grid_points_2d(batch_size, width, height, device=DEVICE):
    """
    Create 2d grid points. Distribute points across a width and height,
    with the resulting coordinates constrained to -1, 1
    returns tensor shape (batch_size, width * height, 2)
    """
    xx, yy = torch.meshgrid(
        [torch.linspace(-1.0, 1.0, height, device=device),
         torch.linspace(-1.0, 1.0, width, device=device)])
    grid = torch.stack([yy, xx], dim=-1).contiguous().view(1, -1, 2).expand(batch_size, -1, -1)  # N, K, 2
    return grid


def batch_noisy_grid(batch_size, width, height, noise_scale=0.1, device=DEVICE):
    """
    Make uniform grid points, and add noise except for edge points.
    """
    grid = batch_grid_points_2d(batch_size, width, height, device)                    # N, K, 2
    mod = torch.zeros([batch_size, height, width, 2], device=device)                  # N, H, W, 2
    noise = (torch.rand([batch_size, height - 2, width - 2, 2]) - 0.5) * noise_scale  # N, H-2, W-2, 2
    mod[:, 1:height - 1, 1:width-1, :] = noise
    return grid + mod.reshape(batch_size, height*width, 2)                            # N, K, 2
