...
class DAVIS(object):
    ...
    def __init__(self, root, year=2016, task='unsupervised', subset='val', sequences='all', resolution='480p', codalab=False):
        """
    ...
    def get_frames(self, sequence):
        for img, msk in zip(self.sequences[sequence]['images'], self.sequences[sequence]['masks']):
            ...
            mask = None if msk is None else Image.open(msk)
            if self.year == '2016' and mask is not None:
    ...
    def get_all_masks(self, sequence, separate_objects_masks=False):
        ...
        ### modified by zhihui, for davis 2016
        if self.year == '2016':
            masks[masks > 0] = 1
